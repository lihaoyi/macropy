...
class MacroCompile(Compile):
    ...
    def __call__(self, source, filename, symbol):
        ...
        required_pkgs = macros._detect_macros(tree)
        for p in required_pkgs:
            __import__(p)

        self.modules.update(sys.modules[p] for p in required_pkgs)

        tree = macros._expand_ast(tree, self.modules)
            
        tree = macros._ast_ctx_fixer.recurse(tree, macros.Load())

        macros.fill_line_numbers(tree, 0, 0)
